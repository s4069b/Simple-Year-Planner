...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    try {
        ...
        if ($action==='blank-next-year') {
            if (!empty($_POST['confirm_blank'])) { yp_blank_next_year(); admin_redirect('Next year blanked.',yp_current_year()+1); }
            throw new RuntimeException('Confirmation was not supplied.');
        }
        if ($action==='toggle-previous-year') {
            $settings=yp_settings();
            $settings['showPreviousYear']=isset($_POST['show_previous']);
            yp_write_settings($settings);
            $redirectYear=(int)($_POST['year'] ?? yp_current_year());
            if (!$settings['showPreviousYear'] && $redirectYear < yp_current_year()) $redirectYear=yp_current_year();
            admin_redirect($settings['showPreviousYear'] ? 'Previous year shown publicly.' : 'Previous year hidden from public view.',$redirectYear);
        }
...
<?php if($yearFrozen):?><p class="notice"><strong><?=$selectedYear?> is frozen.</strong> Previous-year planners are read-only and cannot be edited.</p><form method="post" class="row"><input type="hidden" name="csrf" value="<?=htmlspecialchars($_SESSION['csrf'])?>"><input type="hidden" name="action" value="toggle-previous-year"><input type="hidden" name="year" value="<?=$selectedYear?>"><label><input type="checkbox" name="show_previous" value="1" <?=$settings['showPreviousYear']?'checked':''?>> Show <?=$selectedYear?> to public visitors</label><button>Save visibility</button></form><?php elseif($selectedYear===yp_current_year()+1 && !$yearPresent):?><p class="notice">The <?=$selectedYear?> planner has not been created.</p><form method="post" class="year-copy-form" onsubmit="if(!confirm('Create <?=$selectedYear?> by copying <?=yp_current_year()?> calendar and shading settings?'))return false;showYearWorking('Preparing <?=$selectedYear?>…','Copying calendar and shading settings. Please keep this page open.');return true;"><input type="hidden" name="csrf" value="<?=htmlspecialchars($_SESSION['csrf'])?>"><input type="hidden" name="action" value="copy-next-year"><input type="hidden" name="year" value="<?=$selectedYear?>"><input type="hidden" name="confirm_copy" value="1"><button class="primary">Copy <?=yp_current_year()?> into <?=$selectedYear?></button></form><?php elseif($selectedYear===yp_current_year()+1):?><p class="notice"><strong>Reset options:</strong> either replace <?=$selectedYear?> with a fresh copy of <?=yp_current_year()?>, or blank <?=$selectedYear?> completely.</p><div class="row"><form method="post" class="year-copy-form" onsubmit="if(!(confirm('Replace <?=$selectedYear?> calendars and shading with <?=yp_current_year()?>?') && confirm('Final confirmation: reset <?=$selectedYear?> now?')))return false;showYearWorking('Preparing <?=$selectedYear?>…','Copying calendar and shading settings. Please keep this page open.');return true;"><input type="hidden" name="csrf" value="<?=htmlspecialchars($_SESSION['csrf'])?>"><input type="hidden" name="action" value="reset-next-year"><input type="hidden" name="year" value="<?=$selectedYear?>"><input type="hidden" name="confirm_reset" value="1"><button class="danger">Copy <?=yp_current_year()?> again</button></form><form method="post" class="year-copy-form" onsubmit="if(!(confirm('BLANK <?=$selectedYear?>? All calendars, shading and planner introduction will be removed.') && confirm('Final confirmation: blank <?=$selectedYear?> now?')))return false;showYearWorking('Blanking <?=$selectedYear?>…','Clearing the next-year planner. Please keep this page open.');return true;"><input type="hidden" name="csrf" value="<?=htmlspecialchars($_SESSION['csrf'])?>"><input type="hidden" name="action" value="blank-next-year"><input type="hidden" name="year" value="<?=$selectedYear?>"><input type="hidden" name="confirm_blank" value="1"><button class="danger">Blank <?=$selectedYear?></button></form></div><?php endif;?>
